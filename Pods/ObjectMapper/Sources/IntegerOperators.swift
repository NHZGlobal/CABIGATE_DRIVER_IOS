...
private func toSignedInteger<T: SignedInteger>(_ value: Any?) -> T? {
	...
	if T.self ==   Int.self, let x = Int(exactly: number.uint64Value) {
		return T.init(x)
	}
	if T.self ==  Int8.self, let x = Int8(exactly: number.uint64Value) {
		return T.init(x)
	}
	if T.self == Int16.self, let x = Int16(exactly: number.uint64Value){
		return T.init(x)
	}
	if T.self == Int32.self, let x = Int32(exactly: number.uint64Value) {
		return T.init(x)
	}
	if T.self == Int64.self, let x = Int64(exactly: number.uint64Value) {
		return T.init(x)
	}
